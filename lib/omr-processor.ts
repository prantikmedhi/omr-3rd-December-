...
import type { OpenCVInstance, OpenCVMat, OpenCVRect, OpenCVPoint, OpenCVSize, OpenCVScalar, OpenCVMatVector } from './opencv-types'
 ...
 */
function assessImageQuality(cv: OpenCVInstance, src: OpenCVMat): ImageQualityMetrics {
  const gray = new cv.Mat()
  cv.cvtColor(src, gray, cv.COLOR_RGBA2GRAY, 0)
  ...
  const contrast = stddev.data64F ? stddev.data64F[0] : 0 // Standard deviation as contrast measure
  ...
  cv.Sobel(blurredForSharpness, sobel, cv.CV_32FC1, 1, 1, 3)
  
  const sobelMean = new cv.Mat()
  const sobelStddev = new cv.Mat()
  cv.meanStdDev(sobel, sobelMean, sobelStddev)
  const sharpness = sobelStddev.data64F ? sobelStddev.data64F[0] : 0
 ...
 */
function estimateNoiseLevel(cv: OpenCVInstance, gray: OpenCVMat): number {
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    const variance = stddev.data64F ? stddev.data64F[0] : 0
...
function applyAdvancedPreprocessing(
  ...
  const avgBrightness = mean.data64F ? mean.data64F[0] : 128
...
function detectPaperAtScale(
  ...
  const avgIntensity = mean.data64F ? mean.data64F[0] : 128
  const intensityStd = stddev.data64F ? stddev.data64F[0] : 0
  ...
  cv.findContours(dilated, contours as any, hierarchy, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE)
  matsToCleanup.push(contours, hierarchy)

  let maxArea = 0
  let biggestApprox: OpenCVMat | null = null
  let fallbackContour: OpenCVMat | null = null
  let fallbackArea = 0
  ...
  for (let i = 0; i < contours.size(); ++i) {
    ...
    if (area > minArea && area < maxAreaLimit) {
      ...
      // Fallback: If no 4-point poly yet, track largest contour
      if (area > fallbackArea) {
        fallbackArea = area
        if (fallbackContour) fallbackContour.delete()
        fallbackContour = c.clone()
      }
  ...
  // If no perfect quadrilateral found, try to use the fallback contour
  if (!biggestApprox && fallbackContour) {
    // Create approx from fallback
    const hull = new cv.Mat()
    cv.convexHull(fallbackContour, hull, false, true)
    
    // Simply take the 4 extreme points from the hull to form a quad
    // We need to convert hull (which is just points) to a format orderPoints can use
    // or manually find extreme points.
    
    // Simplify hull first
    const approxHull = new cv.Mat()
    const peri = cv.arcLength(hull, true)
    cv.approxPolyDP(hull, approxHull, peri * 0.02, true)
    
    if (approxHull.rows >= 4) {
      biggestApprox = approxHull.clone()
    } else {
        // If hull simplification is too aggressive, just use hull
         biggestApprox = hull.clone()
    }
  ...
  if (biggestApprox) {
    try {
      // If biggestApprox has != 4 points, we need to extract 4 corners
      let points: {x:number, y:number}[] = []
      
      if (biggestApprox.rows === 4) {
        points = orderPoints(biggestApprox)
      } else {
        // Extract 4 extreme corners from the contour/hull
        points = orderPoints(biggestApprox)
      }
...
function orderPoints(approx: OpenCVMat): { x: number; y: number }[] {
  ...
  // Iterate over all points in the approximation/hull
  ...
  let minDiff = tl.y - tl.x
  let maxDiff = bl.y - bl.x
...
function detectBubblesAdvanced(
  ...
  const contours = new cv.MatVector()
  const hierarchy = new cv.Mat()
  cv.findContours(thresh, contours as any, hierarchy, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
  matsToCleanup.push(contours, hierarchy)
...
function calculateFillPercentageAdvanced(
  ...
  try {
    ...
    const std = stddev.data64F ? stddev.data64F[0] : 0
    mean.delete()
    stddev.delete()
